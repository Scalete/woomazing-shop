    ...
    return (
        <ul className="cart-row">
            <li className="cart-cell">
                ...
                <Link className='hover-underline-animation' to={`/product?_id=${_id}`}>{title}, <br /> <span className={extraOptions.color}>{renderCartColors(extraOptions.color)}</span>, <strong style={{fontWeight: '700'}}>{extraOptions.size}</strong></Link>
            </li>
            <li className="cart-cell">
